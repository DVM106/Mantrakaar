...
function createTextTexture(gl, text, font = 'bold 30px monospace', color = 'black') {
  ...
  // Clear to transparency but preserve RGB color for edge anti-aliasing
  ...
  createShader() {
    ...
    this.program = new Program(this.gl, {
      ...
      fragment: `
        ...
        void main() {
          // Safe aspect ratio calculations to avoid division by zero
          float imgAspect = (uImageSizes.x > 0.0 && uImageSizes.y > 0.0) ? (uImageSizes.x / uImageSizes.y) : 1.0;
          float planeAspect = (uPlaneSizes.y > 0.0) ? (uPlaneSizes.x / uPlaneSizes.y) : 1.0;
          
          // Fit/contain the image inside the plane with a padding/margin based on uFitScale.
          vec2 ratio = vec2(
            max(planeAspect / imgAspect, 1.0),
            max(imgAspect / planeAspect, 1.0)
          ) / uFitScale;
          ...
          // Light card background (white with 90% opacity)
          vec4 cardBg = vec4(1.0, 1.0, 1.0, 0.90);
          vec4 color = cardBg;
          
          if (uv.x >= 0.0 && uv.x <= 1.0 && uv.y >= 0.0 && uv.y <= 1.0) {
            vec4 texColor = texture2D(tMap, uv);
            // Blend texture over card background based on texture alpha
            color = mix(cardBg, texColor, texColor.a);
          }
...
class App {
  ...
  update() {
    if (!this.isDown && !this.isHovered) {
      this.scroll.target += this.autoScrollSpeed; // continuous auto-scroll
    }
  ...
  addEventListeners() {
    ...
    // Drag move and up on window to allow dragging outside container
    window.addEventListener('mousemove', this.boundOnTouchMove);
    window.addEventListener('mouseup', this.boundOnTouchUp);
    window.addEventListener('touchmove', this.boundOnTouchMove);
    window.addEventListener('touchend', this.boundOnTouchUp);

    // Initial drag start scoped to container only to prevent page scroll hijack
