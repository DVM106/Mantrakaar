...
function initializeMantrakaar() {
  ...
  /* ==========================================================================
     ...
     ========================================================================== */
  /* ==========================================================================
     Navbar Search Control & Real-time Suggestions Engine
     ========================================================================== */
  function initNavbarSearch() {
    const searchContainers = document.querySelectorAll('.nav-search-container');
    if (!searchContainers.length) return;

    // Comprehensive Site Search Index
    const searchIndex = [
      // Pages
      { title: 'Home', url: 'index.html', tag: 'page', desc: 'Digital agency homepage, vision & portfolio showcase' },
      { title: 'About Us', url: 'about.html', tag: 'page', desc: 'Who we are, team, ambition & company background' },
      { title: 'All Projects', url: 'projects.html', tag: 'page', desc: 'Complete portfolio showcase across 44+ global clients' },
      { title: 'Career', url: 'career.html', tag: 'page', desc: 'Job openings, company culture, & career opportunities' },
      { title: 'Blog', url: 'blog.html', tag: 'page', desc: 'Articles, design trends, digital insights, & strategy' },
      { title: 'Our Clients', url: 'clients.html', tag: 'page', desc: 'Partner businesses & showcase of global brand work' },
      { title: 'Testimonials', url: 'testimonials.html', tag: 'page', desc: 'Client reviews, ratings & experience feedback' },
      { title: 'Contact Us', url: 'contact.html', tag: 'page', desc: 'Get in touch, global office locations & project inquiries' },

      // Services
      { title: 'UX/UI Design', url: 'design.html', tag: 'service', desc: 'Brand & Interface Design, fluid UX journeys, cohesive design systems' },
      { title: 'Website Development', url: 'development.html', tag: 'service', desc: 'Frontend & core web dev, fast responsive websites from scratch' },
      { title: 'Marketing', url: 'marketing.html', tag: 'service', desc: 'Growth, SEO, PPC, social media marketing, & performance campaigns' },
      { title: 'Consultancy & Audits', url: 'consultancy.html', tag: 'service', desc: 'Architectural analysis, codebase audits, & product strategy' },

      // Projects
      { title: 'PBC Distributors', url: 'project-details.html?id=pbc-distributors', tag: 'project', desc: 'Enterprise distribution web portal' },
      { title: 'U.S. Elevators Co.', url: 'project-details.html?id=u-s-elevators-co', tag: 'project', desc: 'Elevator solutions web application' },
      { title: 'Sanemi', url: 'project-details.html?id=sanemi', tag: 'project', desc: 'E-commerce & brand website solution' },
      { title: 'Pure Mist', url: 'project-details.html?id=pure-mist', tag: 'project', desc: 'Consumer brand digital experience' },
      { title: 'Trade4Wealth', url: 'project-details.html?id=trade4wealth', tag: 'project', desc: 'Financial services web platform' },
      { title: 'Nexus Wizard', url: 'project-details.html?id=nexus-wizard', tag: 'project', desc: 'SaaS product interface design' },

      // Articles
      { title: 'Building a Human Brand: "About Us" Section', url: 'blog.html', tag: 'article', desc: 'The heartbeat of your website\'s brand story' },
      { title: 'Why Doctors Need a Strong Online Presence', url: 'blog.html', tag: 'article', desc: 'Building trust & credibility for specialist medical practices' },
      { title: 'How SEO Elevates Specialist Doctors Online', url: 'blog.html', tag: 'article', desc: 'Search engine optimization strategies for healthcare' },
      { title: 'Indian Manufacturers & Digital Transformation', url: 'blog.html', tag: 'article', desc: 'Industrial web development & growth strategy' }
    ];
    ...
    searchContainers.forEach(container => {
      ...
      // Create Suggestions Dropdown element if missing
      let suggestionsMenu = container.querySelector('.nav-search-suggestions');
      if (!suggestionsMenu) {
        suggestionsMenu = document.createElement('div');
        suggestionsMenu.className = 'nav-search-suggestions';
        container.appendChild(suggestionsMenu);
      }
      ...
      function closeSearch() {
        ...
        suggestionsMenu.classList.remove('active');
      ...
      searchInput.addEventListener('input', () => {
        ...
        if (matches.length === 0) {
          suggestionsMenu.innerHTML = `<div class="search-no-results">No matches found for "<strong>${escapeHTML(query)}</strong>"</div>`;
          suggestionsMenu.classList.add('active');
          return;
        }

        suggestionsMenu.innerHTML = matches.slice(0, 7).map((item, idx) => `
          <a href="${item.url}" class="search-suggestion-item" data-index="${idx}">
            <span class="search-suggestion-tag ${item.tag}">${item.tag}</span>
            <div class="search-suggestion-info">
              <div class="search-suggestion-title">${highlightQuery(item.title, query)}</div>
              <div class="search-suggestion-desc">${highlightQuery(item.desc, query)}</div>
            </div>
          </a>
        `).join('');
      ...
      function highlightQuery(text, query) {
        ...
        return escapeHTML(text).replace(regex, '<strong style="color: var(--brand-purple, #aa00d3); font-weight: 700;">$1</strong>');
      }
